...
for feature in data
    ...
    d = get(props, :IG_DATE, nothing)
    (isnothing(a) || isnothing(d) || a <= 0) && continue
    push!(years, year(unix2datetime(d / 1000)))
    push!(acres, Float64(a))
end

fig = Figure(size=(800, 500))
ax = Axis(fig[1, 1], title="MTBS: 50 Largest Fires on Record",
    xlabel="Year", ylabel="Acres")
if !isempty(years)
    scatter!(ax, years, acres, markersize=sqrt.(acres) ./ 80, color=(:firebrick, 0.6))
end
fig
```
